...
export const StoryHistoryController = {
    async findById(req: Request, res: Response) {
        try {
            ...
            const history = await StoryHistoryService.findById(id);
            if (!history) {
    ...
    async findByChildAndStory(req: Request, res: Response) {
        try {
    ...
    async findByChild(req: Request, res: Response) {
        try {
            const childId = Number(req.params.childId);
            return res.json(
                await StoryHistoryService.findByChild(childId)
            );
        } catch (error) {
            return res.status(500).json({
                message: "Error finding child history"
            });
        }
    },
    async create(req: Request, res: Response) {
        try {
            ...
            return res.status(500).json({
                message: "Error creating story history"
            });
        }
    },
    async nextPage(req: Request, res: Response) {
        try {
            const id = Number(req.params.id);
            const history =
                await StoryHistoryService.nextPage(id);
            return res.json(history);
        } catch (error) {
